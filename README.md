...
- **Bottles (Contributor - Merged in v64.1):** Refactored the global `Manager` class lifecycle to completely isolate runtime states between the main GUI and ephemeral CLI invocations. Implemented deterministic guardrails to prevent concurrent mutations of shared SQLite databases (`playtime_tracker`), covering the isolation layer with automated hermetic test suites using `GSettingsStub`, `pytest-mocker`, and elastic time-freezing via `freezegun`.
- **linux-wallpaperengine (Contributor - Merged Upstream):** Resolved critical Wayland focus-blindness bugs for passive layer-surfaces under modern tiling compositors (Niri/COSMIC in PR #576) by designing an active viewport dispatcher and geometric center fallback. Rewrote the metadata ingestion engine (`PropertyParser` in PR #581) to tolerate dynamic directory parameters and provide safe defaults for missing boolean keys, backed by native `Catch2` unit tests.
